...
-- Ensure subscribers table has proper restrictions (financial subscription data)
CREATE POLICY IF NOT EXISTS "Prevent unauthorized subscriber access" ON public.subscribers
FOR SELECT
USING (user_id = auth.uid() OR auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Prevent unauthorized subscriber inserts" ON public.subscribers  
FOR INSERT
WITH CHECK (user_id = auth.uid() OR auth.role() = 'service_role');

-- Add policy to prevent unauthorized access to user_license_subscriptions
CREATE POLICY IF NOT EXISTS "Restrict license subscription deletes" ON public.user_license_subscriptions
FOR DELETE
USING (
  EXISTS (
    SELECT 1 FROM public.organization_members om
...
CREATE POLICY IF NOT EXISTS "Prevent unauthorized exemption deletes" ON public.billing_exemptions
FOR DELETE  
USING (
  EXISTS (
    SELECT 1 FROM public.organization_members om
