...
-- Make sure equipment_attributes have proper RLS policies
          ...
          AND (
            ...
            EXISTS (
              ...
              WHERE t.id = e.team_id
                ...
                AND tr.role IN ('manager', 'technician', 'owner', 'admin', 'creator')
            )
          )
      )
    );
        ...
        WHERE e.id = equipment_attributes.equipment_id
          AND (
            ...
            EXISTS (
              ...
              WHERE t.id = e.team_id
                AND au.auth_uid = auth.uid()
                AND tr.role IN ('manager', 'owner', 'admin', 'creator')
            )
          )
      )
    );
...
-- Fix type issue with auth_uid in app_user table if needed
