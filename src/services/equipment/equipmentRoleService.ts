...
export type EquipmentUserRole = 'manager' | 'technician' | 'requestor' | 'viewer' | 'none';

export async function getUserRoleForEquipment(equipmentId: string): Promise<EquipmentUserRole> {
  try {
    ...
    console.error('Error getting user role for equipment:', error);
    return 'none';
  }
}
